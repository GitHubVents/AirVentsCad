    ...
    public partial class MakeDxfExportPartDataClass
    {
        ...
        public string SearchDoc(string filePath, SwDocType swDocType)
        {
            try
            {
                ...
                {
                    LoggerMine.Info(String.Format("Файл с именем {0} не найден!", like + filePath + extenison), "", "SearchDoc");
                    return "";
                }
            ...
            {
              LoggerMine.Error(string.Format("Ошибка: {1} Строка: {0}", exception.StackTrace, exception.ToString()), exception.TargetSite.Name, "SearchDoc");
            }
