...
namespace VentsCadLibrary
{
    public class SwEpdm
    {
        ...
        /// <summary>
        /// Registration or unregistation file by path file and registration status.
        /// </summary>
        /// <param name="filePath"></param>
        /// <param name="isRegistration"></param>
        public static void CheckInOutPdm(string filePath, bool isRegistration)
        {
            try
            {
                IEdmFolder5 folderPdm;
                IEdmFile5 filePdm = edmVault5.GetFileFromPath(filePath, out folderPdm);
                if (isRegistration)
                {
                ...
                {
                    int PerentWind_hide = 0;
                    filePdm.LockFile(folderPdm.ID, PerentWind_hide);
                }
            }
            catch (Exception ex)
            {
        ...
        internal static void BatchAddFiles(List<VaultSystem.VentsCadFile> filesList, EdmVault5 edmVault5)
        {
           goto m1;
            ...
            IEdmVault7 edmVault7 =  edmVault5;
        ...
        public static void BatchGet(IEdmVault7 vault, List<AddinConvertTo.Classes.FilesData.TaskParam> files)
        {
            try
            {
                ...
                {                    
                    ...
                    {
                        Логгер.Ошибка($"Message - {ex.ToString()}\nStackTrace - {ex.StackTrace}", null, "BatchUnLock", "SwEpdm");                        
                        continue;
                    }                    
